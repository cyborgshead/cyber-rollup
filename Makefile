...
HTTPS_GIT := https://github.com/cyborgshead/cyber-rollup.git
...
build: go.sum
ifeq ($(OS),Windows_NT)
	$(error cyber server not supported. Use "make build-windows-client" for client)
	exit 1
else
	go build -mod=readonly $(BUILD_FLAGS) -o build/cyber ./cmd/cyber
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/cyber.exe ./cmd/cyber

install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/cyber
...
format: format-tools
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "./tests/system/vendor*" -not -path "*.git*" -not -path "./client/lcd/statik/statik.go" | xargs gci write --skip-generated -s standard -s default -s "prefix(cosmossdk.io)" -s "prefix(github.com/cosmos/cosmos-sdk)" -s "prefix(github.com/cyborgshead/cyber-rollup)" --custom-order
